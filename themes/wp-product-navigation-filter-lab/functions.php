 ...
 * 5. 分类导航、内容、SEO 和演示数据
 *
 * 与筛选直接相关的代码已拆分到 inc/filter-*.php，
 * 便于按“配置 → 请求 → 查询 → 计数 → AJAX”的顺序学习。
 */
...
foreach ($filter_modules as $filter_module) {
    require_once get_theme_file_path('/inc/' . $filter_module);
}
        ...
        wp_localize_script(
            ...
            [
                ...
                'showPerformance' => (
                    ...
                    && defined('PFL_FILTER_DEBUG')
                    && PFL_FILTER_DEBUG
                ),
                'messages' => [
                    'loading' => '正在更新产品结果与筛选计数……',
                    'error'   => 'AJAX 请求失败，可重新请求或使用普通页面打开。',
                ],
            ]
        );
    }
